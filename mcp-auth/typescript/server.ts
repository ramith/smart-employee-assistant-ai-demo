...
let currentAuth: AuthInfo | undefined;

// Helper to check scope (uses req.auth populated by SDK middleware)
function requireScope(scope: string): { error: true; response: any } | null {
    if (!currentAuth?.scopes.includes(scope)) {
        console.log(`[SCOPE] Denied: required=${scope}, has=${currentAuth?.scopes}`);
...
app.post('/mcp', authMiddleware, async (req, res) => {
    // SDK populates req.auth with AuthInfo from our verifier
    currentAuth = req.auth;
    console.log(`[REQUEST] ${req.body?.method} | scopes: ${req.auth?.scopes.join(', ')}`);

    const transport = new StreamableHTTPServerTransport({ sessionIdGenerator: undefined, enableJsonResponse: true });
    res.on('close', () => { currentAuth = undefined; transport.close(); });
