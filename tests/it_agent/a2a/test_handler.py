"""Tests for it_agent/a2a/handler.py — Wave 7, Sprint 1.

Coverage (7 tests) — mirror of tests/hr_agent/a2a/test_handler.py
------------------------------------------------------------------
 1. ``build_it_a2a_router`` returns a FastAPI ``APIRouter``.
...
Design notes
------------
- Identical bootstrap, stub, and patching strategy to ``test_handler.py`` for
  hr_agent.  The only difference is the module paths and type names (IT*).
"""
...
# it_agent.config needs ITAgentConfig exported.
if "it_agent.config" not in sys.modules:
    _cfg_mod = types.ModuleType("it_agent.config")
    _cfg_mod.__package__ = "it_agent"

    class _ITAgentConfig:  # type: ignore[no-redef]
        """Sentinel — tests supply _FakeITAgentConfig."""

    _cfg_mod.ITAgentConfig = _ITAgentConfig  # type: ignore[attr-defined]
    sys.modules["it_agent.config"] = _cfg_mod

# it_agent.ciba.orchestrator needs ITDispatcher exported so handler.py can
# import it at module load time.  We provide a sentinel class; tests use
# StubITDispatcher for actual dispatch behaviour.
if "it_agent.ciba.orchestrator" not in sys.modules:
    _orch_mod = types.ModuleType("it_agent.ciba.orchestrator")
    _orch_mod.__package__ = "it_agent.ciba"

    class _ITDispatcher:  # type: ignore[no-redef]
        """Sentinel — replaced by StubITDispatcher in tests."""

    _orch_mod.ITDispatcher = _ITDispatcher  # type: ignore[attr-defined]
    _orch_mod.ITDispatcherDeps = object  # type: ignore[attr-defined]
    sys.modules["it_agent.ciba.orchestrator"] = _orch_mod
...
from it_agent.a2a.handler import ITA2AHandlerDeps, build_it_a2a_router  # noqa: E402
...
_TRUSTED_ORCH_SUB = "orch-agent-uuid-it-0001"
_USER_SUB = "user-sub-it-abc"
_INBOUND_AUD = "it_agent-oauth-client-id-001"
...
_GOOD_BEARER = "Bearer valid.it.token"
_UNTRUSTED_BEARER = "Bearer untrusted.it.token"
...
_UNTRUSTED_CLAIMS = JWTClaims(
    ...
    exp=9999999999,
    iat=1700000000,
    jti="jti-it-untrusted-001",
    act={"sub": "attacker-uuid-0000"},
    scope="openid orchestrate",
    aut="APPLICATION_USER",
)

_CANNED_RESULT = ResultPayload(
    data={"assets": ["laptop-001", "monitor-002"]},
    token_jti="jti-it-result-001",
...
async def _stub_validate(
    ...
    raw = token.split(" ", 1)[-1]
    if raw == "valid.it.token":
        return _GOOD_CLAIMS
    if raw == "untrusted.it.token":
...
# ---------------------------------------------------------------------------
# Fake ITAgentConfig dataclass (avoids env-var loading in tests)
# ---------------------------------------------------------------------------


@dataclass(frozen=True)
class _FakeITAgentConfig:
...
def _make_deps(
    ...
    dispatcher = StubITDispatcher()
    return ITA2AHandlerDeps(
        config=_FakeITAgentConfig(),  # type: ignore[arg-type]
        dispatcher=dispatcher,  # type: ignore[arg-type]
        pending=pending if pending is not None else {},
    )


def _build_app(deps: ITA2AHandlerDeps) -> FastAPI:
    app = FastAPI()
    app.include_router(build_it_a2a_router(deps))
    return app
...
class TestBuildItA2ARouter:
    """Static structure tests — no async / HTTP needed."""

    def test_returns_api_router(self) -> None:
        """TC-1: build_it_a2a_router returns a FastAPI APIRouter instance."""
        deps = _make_deps()
        router = build_it_a2a_router(deps)
    ...
    def test_trusted_orchestrator_subs_matches_config(self) -> None:
        ...
        with patch("it_agent.a2a.handler.build_a2a_router", side_effect=_capture):
            build_it_a2a_router(deps)
...
class TestItA2ARouterEndToEnd:
    ...
    async def test_valid_token_dispatches_and_returns_result(self) -> None:
        ...
        with _patch_validate():
            async with AsyncClient(
                ...
                resp = await client.post(
                    "/a2a/message/send",
                    json=_send_body("it.list_available_assets"),
        ...
        assert result["data"] == {"assets": ["laptop-001", "monitor-002"]}
        assert result["token_jti"] == "jti-it-result-001"
        dispatcher: StubITDispatcher = deps.dispatcher  # type: ignore[assignment]
        assert len(dispatcher.calls) == 1
        assert dispatcher.calls[0]["tool"] == "it.list_available_assets"
