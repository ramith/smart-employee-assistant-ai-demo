...
# ---------------------------------------------------------------------------
# V-HR-11 / V-HR-12: Sprint 3 3A.3 — Step 7 denylist enforcement
# ---------------------------------------------------------------------------
#
# Per the 2026-05-10 deferral lock, Sprint 3 3A.3 ships denylist-only
# (introspection deferred to Sprint 4). These two tests collapse the original
# 6-case cache×denylist matrix from NIT-10 into the minimum viable check:
# (a) jti present in denylist → ScopeError(ERR-MCP-002)
# (b) jti absent → claims returned normally
#
# A third companion test (R-LOGOUT-5) lives in tests/integration and exercises
# the same path through the live HTTP route.
...
async def test_denylist_hit_raises_err_mcp_002(
    ...
    """V-HR-11: jti in denylist → ScopeError(ERR-MCP-002) with reason=denylist_hit."""
    _, public_jwk = rsa_keypair
    token = sign_token(base_payload)
    validator = _make_validator(base_validation_config, public_jwk)

    # Wire a RevocationState whose denylist contains the test jti.
...
async def test_denylist_miss_passes_through(
    ...
    """V-HR-12: jti not in denylist → claims returned normally (Step 7 no-op)."""
    _, public_jwk = rsa_keypair
    token = sign_token(base_payload)
    validator = _make_validator(base_validation_config, public_jwk)

    # Empty denylist. A different jti is present, so the test jti misses.
    from common.revocation import RevocationState
    revocation = RevocationState()
