"""Tests for hr_server/config.py — Wave 4, Sprint 1.
...
# hr_server directory may have legacy dotenv import; load config.py directly
_ensure_pkg("hr_server")
_load_module("hr_server.config", "hr_server/config.py")
...
def _base_env() -> dict[str, str]:
    return {
        ...
        "HR_SERVER_EXPECTED_AUD": "hr-oauth-client-id",
    }
...
class TestSuccessfulConstruction:
    ...
    def test_basic_fields(self) -> None:
        ...
        assert cfg.expected_aud == "hr-oauth-client-id"
    ...
    def test_explicit_auth_issuer_used_when_set(self) -> None:
        env = {
            **_base_env(),
            "AUTH_ISSUER": "https://is.example.com:9443/oauth2/token",
        }
        cfg = HRServerConfig.from_env(env)
        assert cfg.is_issuer == "https://is.example.com:9443/oauth2/token"

    def test_custom_port_parsed(self) -> None:
        env = {**_base_env(), "HR_SERVER_PORT": "9000"}
        cfg = HRServerConfig.from_env(env)
    ...
    def test_required_scopes_overridable(self) -> None:
        env = {**_base_env(), "HR_SERVER_REQUIRED_SCOPES": "hr.read,hr.write"}
        cfg = HRServerConfig.from_env(env)
        assert cfg.required_scopes == frozenset({"hr.read", "hr.write"})
    ...
    def test_introspect_disabled_when_false(self) -> None:
        env = {**_base_env(), "HR_SERVER_INTROSPECT_ENABLED": "false"}
        cfg = HRServerConfig.from_env(env)
...
class TestN28StartupLog:
    ...
    def test_expected_aud_in_startup_log(self, caplog: pytest.LogCaptureFixture) -> None:
        env = {**_base_env(), "HR_SERVER_EXPECTED_AUD": "hr-oauth-client-id-xyz"}
        with caplog.at_level(logging.INFO, logger="hr_server.config"):
            HRServerConfig.from_env(env)
        assert "hr-oauth-client-id-xyz" in caplog.text

    def test_token_enforcement_label_in_startup_log(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
        with caplog.at_level(logging.INFO, logger="hr_server.config"):
            HRServerConfig.from_env(_base_env())
        assert "token enforcement active" in caplog.text

    def test_trusted_act_subs_in_startup_log(
        self, caplog: pytest.LogCaptureFixture
    ) -> None:
        env = {**_base_env(), "HR_SERVER_TRUSTED_PEER_AGENTS": "hr_agent-uuid-n28"}
        with caplog.at_level(logging.INFO, logger="hr_server.config"):
            HRServerConfig.from_env(env)
        assert "hr_agent-uuid-n28" in caplog.text
...
class TestFrozensetParsing:
    ...
    def test_trusted_act_subs_parsed(self) -> None:
        env = {
            **_base_env(),
            "HR_SERVER_TRUSTED_PEER_AGENTS": "uuid-hr-1, uuid-hr-2 ,uuid-hr-3",
        }
        cfg = HRServerConfig.from_env(env)
        assert cfg.trusted_act_subs == frozenset({"uuid-hr-1", "uuid-hr-2", "uuid-hr-3"})
...
class TestPortParsing:
    ...
    def test_non_integer_port_raises(self) -> None:
        env = {**_base_env(), "HR_SERVER_PORT": "notaport"}
        with pytest.raises(ValueError, match="HR_SERVER_PORT"):
            HRServerConfig.from_env(env)
