...
| `ERR-LLM-003` | OpenAI/WSO2 AI Gateway returns 5xx / quota / network error; keyword fallback also disabled or doesn't match. | INFRA + LLM | High | "I'm having trouble thinking right now. Please try again in a moment." | Retry. | `error_id=ERR-LLM-003 phase=llm_route outcome=llm_unavailable provider=openai status=<http> fallback_attempted=<bool> request_id=<rid>` | 503 |
