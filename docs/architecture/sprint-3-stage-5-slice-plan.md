...
|---|---|---|---|---|---|
| **3A.0** spikes | Day 1 (½) | C14 + C13 capability probes; outcome documented as F-20/F-21. | BLOCK-A | — | — |
...
| **3A.3** MCP server enforcement | Day 4 | **Denylist check only** (introspection deferred to Sprint 4 per [`project_introspection_deferred.md`](../../../memory.placeholder) — F-21 broke the backstop story; the operational cost of per-server creds isn't worth a backstop that doesn't fire). `validate_token()` extended with Step 7 denylist hit → 401 `ERR-MCP-002`. | G-6, G-7 (denylist half) | D3.1 (server side) | R-LOGOUT-5 |
...
| **3B.2** binding_message + carries | Day 6 | `binding_message` reason variants; SPA `?reason=admin_terminated` banner; UC-10 walkthrough; F-20 wiring (auth_req_id revoke if C14 PASS). | FIX-15, FIX-17 | D3.4 | — |
...
**Goal:** lock the Stage 4 BLOCK-A open question + close C14 (Q-LOGOUT-4 ghost approval).
...
- Capture outcomes in `docs/architecture/sprint-1-fixes.md` as F-20 (C14) and F-21 (C13).
...
- IF F-20 PASS (C14 capability test), wire `auth_req_id` revoke into the cancel path; otherwise document the "ghost approval" caveat in `demo-runbook.md`.
...
|---|---|---|
| **1** (Mon) | 3A.0 spikes (C14 + C13) → F-20/F-21 | 3A.1 begin: routes.py + logout_handler skeleton |
...
| **6** (Mon) | 3B.1 finish: live admin-terminate manual | 3B.2: binding_messages + C14 wiring (if PASS) |
...
|---|---|---|---|
| C14 — does IS support `auth_req_id` revoke? | `/oauth2/ciba/revoke` POST with `auth_req_id` from a pending CIBA | F-20 PASS / FAIL → drives 3B.2 wiring | Q-LOGOUT-4 ghost-approval mitigation |
...
| # | Question | Lock |
|---|---|---|
| L-1 | Spike day shape | **Serial.** C14 first, then C13. Single operator. |
