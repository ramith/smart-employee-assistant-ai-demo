...
- **D4. Multi-turn cubicle chat carries human-typed strings (cubicle_id, employee_username, employee_email) through the orchestrator → agent → IS → consent widget → SPA pipeline.** The keyword fallback uses regex extraction (`orchestrator/chat/keyword_fallback.py:97`); the LLM path uses OpenAI's structured `tool_calls` (the tool catalogue is injected as `ChatOpenAI.bind_tools()` function schemas — no JSON parsing). Both paths must validate the extracted args before they become CIBA `binding_message` / `action_text`.
