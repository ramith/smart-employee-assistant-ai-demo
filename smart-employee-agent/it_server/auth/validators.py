"""IT-server MCP token validator — Sprint 1 Wave 5.

Mechanical mirror of ``hr_server/auth/validators.py`` with ``HR`` → ``IT``
renames and it-specific scope defaults (``it.read``).

Implements the F-04 canonical 6-step MCP token validation for the it_server
...
F-04 six-step check (in order):
  ...
  4. ``aud == config.expected_aud``  (= IT_AGENT_OAUTH_CLIENT_ID)
...
__all__ = [
    "ITServerTokenValidationConfig",
    "ITServerTokenValidator",
]
...
class ITServerTokenValidationConfig:
    """Immutable configuration driving the 6-step F-04 MCP token validator.

    This is a thin projection of ``ITServerConfig`` that contains only the
    fields the validator needs.  Use ``ITServerTokenValidator.from_config()``
    to construct from a full ``ITServerConfig`` without duplicating field
    references everywhere.

    Attributes:
        expected_iss: Exact issuer URL; must match token ``iss``.
        jwks_url: Full URL to the IS JWKS endpoint.
        expected_aud: The IT-agent's OAuth Client ID.  Every token-B must
            carry this value in its ``aud`` claim (F-04).
        trusted_act_subs: Frozenset of IT-agent UUIDs that may appear as
...
class ITServerTokenValidator:
    """Implements the F-04 6-step MCP token validation for it_server.

    One instance is created per process during startup and injected into
    ``it_server/mcp/tools.py`` via ``build_mcp_app()``.
    ...
    @classmethod
    def from_config(cls, server_config: object) -> "ITServerTokenValidator":
        """Convenience: lift validator fields from a ``ITServerConfig`` instance.

        Args:
            server_config: A ``ITServerConfig`` frozen dataclass (Wave 4).
                Only the fields ``is_issuer``, ``is_jwks_url``, ``expected_aud``,
                ``trusted_act_subs``, ``required_scopes``, and ``is_insecure_tls``
                are accessed; this avoids a hard import of ``it_server/config.py``
                and eases unit-testing with minimal stubs.

        Returns:
            A fully configured ``ITServerTokenValidator``.
        """
        validation_config = ITServerTokenValidationConfig(
    ...
    async def validate_token(
        ...
        *,
        ...
        logger.debug(
            "it_validator_entry expected_aud=%s trusted_act_subs=%s required_scopes=%s",
        ...
        logger.debug(
            "it_validator_claims_decoded jti=%s iss=%s aud=%r act=%r scope=%r",
            claims.jti,
            claims.iss,
            claims.aud,
            claims.act,
        ...
        if effective_scopes:
            ...
            if missing:
                logger.debug(
                    "it_validator_scope_fail required=%s present=%s missing=%s",
                    sorted(effective_scopes),
                    sorted(token_scopes),
                    sorted(missing),
                )
    ...
    def log_startup_assertion(self) -> None:
        ...
        ``IT_AGENT_OAUTH_CLIENT_ID`` without needing to intercept a live
