"""IT-server service configuration.
...
class ITServerConfig:
    """Immutable configuration for the it_server MCP service.
    ...
    Attributes:
        ...
        expected_aud: The IT-agent's OAuth Client ID — every inbound token-B must
            carry this value as ``aud`` (F-04).  Logged at INFO on startup (N28).
        trusted_act_subs: Frozenset of IT-agent UUIDs that may appear as ``act.sub``
    ...
    def from_env(
        ...
        Returns:
            A fully validated :class:`ITServerConfig`.
        ...
        # Server bind
        host = env.get("IT_SERVER_HOST", "0.0.0.0").strip()
        port = _parse_port(env.get("IT_SERVER_PORT", "8004"), "IT_SERVER_PORT")

        # Sprint 2 hooks
        introspect_enabled = _parse_bool(
            env.get("IT_SERVER_INTROSPECT_ENABLED", "true")
        )
...
def load() -> ITServerConfig:
    """Module-level factory: read ``os.environ`` and return a validated config.

    Raises:
        ValueError: Propagated from :meth:`ITServerConfig.from_env`.
    """
    return ITServerConfig.from_env()
