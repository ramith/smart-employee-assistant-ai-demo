...
## Sprint 5 extension — HR Admin lists leave requests via chat (`hr.read_all_leaves`, scope `hr_approve_rest`)

HR Admin can now ask in chat: "show me all the leave requests" or "are there any leave requests I need to approve?" The orchestrator routes the message to `hr.read_all_leaves` via the keyword router (or LLM router). The HR Agent initiates CIBA with scope `openid hr_approve_rest` — the same scope that gates approve/reject — so whoever can approve is who can list. IS issues an OBO token; the HR Agent calls `POST /mcp/tools/get_all_leave_requests` (optional `status` and `employee_name` filters); the result is rendered in chat as a bullet list. Employees lack `hr_approve_rest` → IS denies the CIBA consent (`access_denied`) → the existing ERR-CIBA-005 denial copy surfaces in the SPA ("You declined the consent request" / "you don't have permission"). No `sub` or `user_sub` appears in any row; only display-safe fields (`request_id`, `employee`, `type`, `start_date`, `end_date`, `days_requested`, `status`).
