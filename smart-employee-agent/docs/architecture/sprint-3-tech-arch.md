...
**Stage 4 changes:** retry semantics specified inline (FIX-22); SECURITY-DEGRADED label introduced (FIX-6); backstop honesty depends on F-21 / C13 outcome (BLOCK-A).

| Failure point | Behaviour | Backstop |
|---|---|---|
...
| Orchestrator process restarts mid-cascade | denylist entries on partial-fan-out receivers are lost on their next restart too (Q5 single-process); next MCP call hits introspection. | F-21 PASS: backstop within ≤60 s. F-21 FAIL: SECURITY-DEGRADED until tokens expire naturally. Production roadmap: persistent denylist (Redis). |
...
| **NEW OQ-9 (BLOCK-A)** | **Open — 3A Day 1** | C13 capability spike: does IS introspection return `active=false` for CIBA-issued OBO tokens after token-A revoke? PASS → backstop story holds; FAIL → §5 error matrix re-classified as SECURITY-DEGRADED for half-fan-out + crash scenarios. Document as F-21 in `sprint-1-fixes.md`. Operator runbook: [`docs/spikes/sprint-3-c13-introspection-capability.md`](../spikes/sprint-3-c13-introspection-capability.md). |
