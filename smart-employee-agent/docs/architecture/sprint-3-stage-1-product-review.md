...
| R-6 (Stage 4) | **Introspection backstop for OBO tokens after parent revoke is unverified (BLOCK-A).** §5 error matrix assumes that revoking token-A makes child token-B introspect as inactive. This is the *expected* OIDC behaviour but not empirically proven on WSO2 IS 7.2 for CIBA-issued grants. F-19 already showed CIBA grants are not session-bound for BCL; the parallel may hold for introspection. | **C13 capability spike on 3A Day 1**, parallel with C10. PASS → ship as designed. FAIL → §5 rows 1, 2, 3 re-classified as SECURITY-DEGRADED until natural TTL (1 h); demo runbook surfaces operator-restart action on partial fan-out. Outcome documented as F-21 in `sprint-1-fixes.md`. Runbook: [`docs/spikes/sprint-3-c13-introspection-capability.md`](../spikes/sprint-3-c13-introspection-capability.md). |
