...
| R-6 (Stage 4) | **Introspection backstop for OBO tokens after parent revoke is unverified (BLOCK-A).** ~~Hypothesis to test on 3A Day 1.~~ **Resolved 2026-05-09 — F-21 FAIL** (see [`sprint-1-fixes.md`](sprint-1-fixes.md) §F-21). WSO2 IS 7.2 does not propagate token-A revoke to CIBA OBO tokens. | Per Stage 5 L-2: ship with **SECURITY-DEGRADED labels**. tech-arch §5 rows for `/oauth2/revoke` 5xx, single fan-out leg fail, all-legs fail, and orchestrator restart mid-cascade are all SECURITY-DEGRADED until natural TTL. Demo runbook surfaces operator-restart action on `logout_fanout_partial` WARN. Denylist on the 4 receivers is the **only** revocation primitive for OBO tokens — the gateway pattern is not just preferred, it's required. |
