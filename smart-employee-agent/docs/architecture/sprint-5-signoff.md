...
`1698dfb` S5.2+S5.3 (LLM router + composer, config, card expansion, AIza redaction, wiring) ·
...
- **LLM-driven chat routing + reply composition** (Gemini `gemini-2.5-flash` via `langchain-google-genai`), active when `LLM_FALLBACK_MODE=llm` + `GEMINI_API_KEY` are set in `orchestrator/.env` (gitignored). New `orchestrator/llm/` package: `client.py` (Protocol + dataclasses + `LLMError`, stdlib-only), `prompts.py` (router/composer prompts, `parse_router_output`, `render_outcomes`, `strip_sensitive` — stdlib-only), `router.py` (`resolve_tool_calls` — stdlib + client/prompts), `composer.py` (`compose_reply` — stdlib + client), `gemini.py` (`GeminiLLMClient` — the *only* langchain-importing module, lazily imported by `main.py`).
...
- **Keyword fallback retained** — every Gemini failure mode (timeout, quota, auth, malformed output, langchain not installed) degrades to exactly the Sprint-4 behaviour (keyword router + `_render_result` concatenation), never a hard error. `_run_serial_fan_out` wraps the final compose+publish so a terminal `chat_message` SSE event is *structurally* guaranteed.
- **Security hardening** (Stage-8 findings folded in): `strip_sensitive` drops `sub`/`*_sub`/`issued_by`/`*token*`/`*secret*` keys + UUID-shaped string values from anything that reaches a Gemini prompt or a chat reply (`render_outcomes` + `_render_result`); `hr.lookup_employee` is *off-card* so the LLM can't route to it; new `AIza[0-9A-Za-z_-]{35}` → `<REDACTED_API_KEY>` redaction pattern; `GeminiLLMClient` logs `type(exc).__name__`+`str(exc)[:200]`, never `repr(exc)`; `OrchestratorConfig.from_env` warns (doesn't crash) on llm-mode + no key.
...
- `./scripts/demo-up.sh` → smoke PASSED, 6/6 services healthy; orchestrator log: `llm_client_enabled model=gemini-2.5-flash timeout_s=8.0 max_output_tokens=512`, `llm_fallback_mode=llm`. The orchestrator Docker image builds with `langchain-google-genai` installed.
- `git grep -nP 'AIza[0-9A-Za-z_-]{35}'` over tracked files → nothing but the fake `"AIza"+"x"*35` test sentinel; no real key in git history; `orchestrator/.env` is gitignored (`.gitignore:75 **/.env`).
...
M5 is fully closed when the operator walks [`sprint-5-stage-11-manual-gate.md`](sprint-5-stage-11-manual-gate.md) and ticks the 12-box EC5 checklist — in particular: single- and multi-tool free-text routing produce LLM-worded replies (§1, §2); "annual leave June 10–14" submits a real request that shows in the My Leaves panel (§3 — UC-13 Main flow); "I want leave" with no dates → a clarification, no CIBA wasted (§4); the prompt-injection scenario → zero writes / zero escalation (§6); killing Gemini → keyword fallback within the timeout, demo still works (§8, with `LLM_FALLBACK_MODE` restored to `llm` afterwards). Until then the build is "automated-gate green, manual-gate prepared."

## 4. Carry-overs (from the retro — not blocking M5)

C1 pin langchain exactly + lockfile · C2 pre-commit `AIza…` grep hook · C3 per-Session chat rate limit · C4 chat-history replay · C5 `-m live_llm` in a nightly lane · C6 `it.issue_asset` reply: resolve `employee_id`→username · C7 reconcile the still-open Sprint-4 manual gate + write `sprint-4-signoff.md`. Details in [`sprint-5-retro.md`](sprint-5-retro.md) §4.
