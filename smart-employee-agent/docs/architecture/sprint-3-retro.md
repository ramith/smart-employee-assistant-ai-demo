...
- **WSO2 IS 7.x RC packaging quirks.**
  1. The "MCP Client Application" template doesn't expose two logout-related fields in the modern Console UI: `backChannelLogoutUrl` (BCL) and a `post_logout_redirect_uri`/`callbackURLs` entry for the bare root URI. Without the callback entry, IS returns `oauthErrorCode=access_denied / Post logout URI does not match with registered callback URI` when the SPA redirects to `/oidc/logout` with `post_logout_redirect_uri=http://localhost:8090/`. Both gaps are patched by the same `scripts/set-bcl-url.sh` GET→merge→PUT (using a `regexp=` pattern to cover both the auth-code callback and the logout root in one entry). `check-is-config.py` Section 4b now FAILs if either URL is missing. Underlying schema supports both fields at the IS layer — purely a Console UI template gap.
  2. **Federated logout NPE (`retry.do` after UAE Pass login).** When a user authenticates via a federated OIDC IDP (UAE Pass), IS records the IDP as `AuthenticatedIdP`. On RP-initiated logout, IS propagates the logout to the federated IDP by redirecting the browser to `OIDCLogoutEPUrl` configured on the IDP. The IDP then redirects back to `https://<IS>/commonauth?sessionDataKey=<newId>`. IS tries to resume `<newId>` but the auth context has already been cleaned up during the SP-level logout → `NullPointerException: Cannot invoke "AuthenticationContext.isOrgApplicationLogin()" because "context" is null` in `DefaultRequestCoordinator` → IS shows `retry.do?crId=<id>`. The logout itself **succeeds** (session is cleared in audit.log), but the browser ends up on the IS error page. Diagnosed via `grep <crId> wso2carbon.log audit.log`. Fix: clear `OIDCLogoutEPUrl` on the UAE Pass IDP via the Identity Providers API — IS then clears the local session and immediately redirects to `post_logout_redirect_uri` without the federated round-trip. Documented in `wso2-is-setup.md` Step 2b.
  3. WSO2 IS emits BCL `logout_token` with no `typ` header (spec REQUIRES `logout+jwt`). Soft-checked in our receiver: absent OK, present-and-wrong rejected. Memory: `project_wso2_is_bcl_typ_omission.md`.
