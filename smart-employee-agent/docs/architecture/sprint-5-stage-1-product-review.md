...
- `LLM_FALLBACK_MODE` and `GEMINI_API_KEY` exist in `orchestrator/config.py` but are **not wired to runtime routing**. `orchestrator/main.py` always constructs `KeywordRouter()`; the code comment says *"Sprint 2 will add a branch for llm mode"* — it never happened.
...
- LangChain deps (`langchain`, `langchain-google-genai`, `langchain-mcp-adapters`) are declared in `orchestrator/requirements.txt` per the original milestone plan but unused in the v4 codebase.
...
1. **LLM router** — Gemini selects which specialist tool(s) to call and extracts their arguments from the user's natural-language message. Replaces `KeywordRouter` as the primary path.
2. **LLM reply composer** — Gemini turns the raw tool outputs into one natural-language answer. Replaces the `"\n\n".join(...)` concatenation.
...
4. **Keyword fallback retained** — when Gemini is unreachable, rate-limited, returns malformed output, or the key is invalid, the orchestrator silently falls back to the existing `KeywordRouter` + `_render_result`. The demo never hard-fails on an LLM hiccup. (This is the "fallback" that `LLM_FALLBACK_MODE` always implied.)
...
- **`langchain-mcp-adapters` / full LangChain agent executor.** The v4 topology (orchestrator → A2A → specialist agent → CIBA → MCP) doesn't fit the `MultiServerMCPClient` pattern. S5 uses the LLM purely as router + composer; the existing A2A/CIBA fan-out is unchanged. `langchain-google-genai`'s `ChatGoogleGenerativeAI` is the only LangChain piece used (it's already pinned).
...
|---|------|------------------|
| R1 | Gemini latency adds seconds to every chat turn (router call + composer call = 2 round-trips). | Stage 6 — single combined call where possible; cap `max_output_tokens`; `gemini-2.5-flash` not `pro`; 8s timeout → fallback. |
| R2 | LLM emits a tool_id / agent_id that doesn't exist → dispatch error. | Stage 6 — validate every returned `ToolCall` against `registry` + `_TOOL_REGISTRY` before fan-out; drop unknowns; if all dropped, fall back to keyword router. |
...
| R4 | API key leaks (it's a real Google key). | Lives only in `orchestrator/.env` (gitignored). Never logged. Never in compose `environment:` literals. CI/grep check that no `AIza…` string is committed. |
...
1. With `LLM_FALLBACK_MODE=llm` + a valid `GEMINI_API_KEY`, a free-text chat message routes to the right specialist tool(s) with correct args, runs the CIBA flow, and the SPA shows a natural-language reply.
...
3. Killing the network to Gemini (or an invalid key) → the orchestrator falls back to keyword routing within the timeout; the demo still works (degraded phrasing, same security).
4. No `AIza…` key string anywhere in git history.
