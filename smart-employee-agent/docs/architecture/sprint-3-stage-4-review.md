...
**Resolved 2026-05-09 (empirical + source):** C13 probe FAILed. WSO2 IS source confirms: `revokeAccessTokens(String[])` is single-row UPDATE; `IDN_OAUTH2_ACCESS_TOKEN` schema has no parent/actor/request_id linkage; `act` claim is JWT-body only. Architectural, not a bug. See [`sprint-3-is-source-analysis.md`](../spikes/sprint-3-is-source-analysis.md) §3. Per Stage 5 L-2: ship with SECURITY-DEGRADED labels for user-driven `/oauth2/revoke` paths. Denylist is the only revocation primitive for OBO tokens.
**F-19 addendum (same source dive):** F-19 itself was a probe artifact. With `id_token_hint`, IS DOES walk session participants and fire BCL — `OIDCLogoutServlet.java:272–291` shows two branches; original spike URL hit the no-clientId branch. Locked Q3 design uses `id_token_hint`, so D3.2 admin-terminate gets BCL fan-out as designed.
