...
This sprint added no live network in the strict suite — every LLM call is mocked (`FakeLLMClient` returns canned `RoutedToolCall`s / canned prose / an `LLMError` to raise). The `gemini.py` adapter (the only langchain-touching module) is *not* imported by the suite; a dedicated test asserts the rest of the LLM layer imports without langchain installed (it isn't in the test venv).
...
| `tests/orchestrator/llm/test_router.py` (27) | `parse_router_output`: valid array, empty `[]` (valid), markdown-fenced, list-of-content-blocks, garbage → `LLMError`, mixed valid/malformed → keeps valid, all-malformed-nonempty → `LLMError`. `strip_sensitive`: drops `sub`/`*_sub`/`issued_by`/`*token*`/`*secret*` keys + UUID-shaped string values, recursive into dicts/lists, passes scalars/non-UUID strings. `render_outcomes`: success strips `sub`, failure shows `error_id`+`reason`, empty → `(no tools ran)`. `router_system`: lists every catalogue tool, `hr.lookup_employee` absent. `build_catalogue`: from registry, `args` populated, `hr.lookup_employee` absent. `resolve_tool_calls`: LLM hit → those tools; hallucinated/non-scalar args stripped, valid kept; unknown agent + unknown tool dropped, valid kept; internal-only `hr.lookup_employee` dropped; LLM `LLMError` → keyword fallback; LLM `[]` → keyword fallback; mode=keyword (with a client present) → never calls LLM; no llm_client → keyword. |
...
| `tests/orchestrator/llm/test_no_langchain.py` (1) | With `langchain` / `langchain_core` / `langchain_google_genai` imports blocked, `orchestrator.llm.client/prompts/router/composer` all import fine — and `orchestrator.llm.gemini` raises `ModuleNotFoundError` (proving the langchain dep lives only there, behind the lazy import in `main.py`). |
| `tests/orchestrator/test_config.py` (+5) | `gemini_model` default + override; `llm_timeout_s` / `llm_max_output_tokens` defaults + parse; **`LLM_FALLBACK_MODE=llm` + no key → warns, keeps mode, no exception** (exit-criterion §6.13); blank `LLM_FALLBACK_MODE` → `keyword`. |
| `tests/common/logging/test_redaction.py` (+4) | A `AIza`-shaped key (39 chars) is stripped from a URL in a log line, from a bare `key=…`, and from a `LogRecord` via `RedactionFilter`; the bare word `AIza` (too short) is *not* over-redacted. |
...
| §6.5 kill Gemini → keyword fallback within the timeout, demo still works | unit: `resolve_tool_calls` LLM-error → keyword + `compose_reply` LLM-error → fallback + `_run_serial_fan_out` outer guard publishes a terminal `chat_message`; e2e: Stage 11 (kill-the-network scenario). |
| §6.6 no `AIza…` key string in tracked files | `git grep -nP 'AIza[0-9A-Za-z_-]{35}'` over tracked files → only the fake `"AIza"+"x"*35` sentinel in `test_redaction.py`; nothing in git history. Plus the redaction filter (`test_redaction.py` `TestGoogleApiKeyRedaction`). |
...
| §6.8 `./scripts/demo-up.sh` smoke green; manual gate walked | ✅ smoke green; orchestrator log shows `llm_client_enabled model=gemini-2.5-flash`. Manual gate = Stage 11. |
...
| §6.10 forced Gemini auth error never logs the key | `test_redaction.py` `TestGoogleApiKeyRedaction.test_filter_strips_api_key_from_record` + `GeminiLLMClient` logs `type(exc).__name__`+`str(exc)[:200]`, never `repr(exc)`. (A live forced-error test is `-m live_llm`, not in the strict suite.) |
...
- **The live Gemini round-trip** (router prompt → real model → JSON; composer prompt → real model → prose). Exercised by the Stage-11 manual gate and an opt-in `pytest -m live_llm` smoke (requires `langchain-google-genai` installed + a valid `GEMINI_API_KEY`; deliberately *not* part of `./tools/run-tests.sh`). Rationale: a strict CI suite must not depend on a paid external API / network.
...
- **The orchestrator Docker image building with `langchain-google-genai`** — verified by running `./scripts/demo-up.sh` (orchestrator built; `llm_client_enabled` in the log; 6/6 healthchecks pass), not by a unit test.
