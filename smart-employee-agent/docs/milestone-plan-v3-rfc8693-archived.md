...
- **§2.1 spike**: added P13 (hr_server `aud` compatibility).
...
- **§3.3 token flow**: added Hop 4 — hr_agent → hr_server re-mints via RFC 8693 (resolves `aud` mismatch BLOCK).
- **§3.4 routing**: LLM sees opaque `agent_id` enum (`hr_agent`, `it_agent`); orchestrator maps `agent_id`→canonical URL server-side from a private registry.
...
- **§ Architecture symmetry fix:** added `it_server/` as a sibling to `hr_server/` so IT Asset Agent wraps a real backend (parallel to HR Agent → hr_server) instead of holding asset data in-memory. **Implementation must mirror `hr_server/` exactly** — same in-memory store pattern, same FastAPI+FastMCP wiring, no incidental complexity. Adds **Hop 5** (IT Agent → it_server re-mint, parallel to Hop 4), new spike probe **P14**, new API resource `it_server-api`, tests **N16/N17**. Symmetry tests Hop-4-style re-mint on two independent paths — proves the pattern composes.
...
- Architectural decisions held against architect's "production-shaped" critique: kept `it_server/` + Hop 5 (security-engineer marked LOAD-BEARING; user-confirmed), kept two-layer revocation, kept agent-card non-trust-anchor proofs (N8/N8b), kept full N1/N7/N3 audience+chain test set.
...
```
   ...
   (browser)│           │  (LLM = Gemini,      │            │
            ...
            │   │  hr_server   │            │  it_server   │
            │   │  MCP + REST  │            │  MCP         │
            │   │  (existing)  │            │  (new, mirrors hr_server shape)
            │   └──────────────┘            └──────────────┘
            │
        SPA (client/) ──────► Orchestrator (HTTP/streaming chat)
```

**Services after Sprint 1:** `client/` (existing), `orchestrator/` (extracted from `agent/`), `hr_agent/` (extracted identity layer + new A2A surface; calls `hr_server` via MCP), `it_agent/` (new specialist; calls `it_server` via MCP), `hr_server/` (existing — unchanged), `it_server/` (new — mirrors hr_server shape with in-memory asset store).
...
- v3 (this version, with `it_server/` symmetry): reviewed by architect-reviewer + security-engineer with explicit POC framing. Their findings are integrated below.
...
|---|---|---|
| **P0** *(new)* | Does the documented 3-step **App-Native Auth flow** (`/oauth2/authorize` → `/oauth2/authn` → `/oauth2/token` with `response_mode=direct`) work for an Asgardeo Agent identity, using its **OAuth App client_id** + **Agent ID/Secret**? Replaces the `client_credentials` approach we originally assumed. | A valid access_token comes back; we use this as `actor_token` in P1. |
| P1 | Does Asgardeo's RFC 8693 token-exchange grant accept `actor_token` (the App-Native-Auth-derived token from P0) and populate the `act` claim? | `act.sub == orchestrator AGENT ID` present in returned JWT. |
...
| **P10** | Does Asgardeo's authorize endpoint accept `requested_actor=<AGENT_ID>` (Pattern C) and produce a delegated user token with `act.sub` populated? Is `actor_token` in Authorization header at code exchange ALSO required, or does `requested_actor` alone suffice? | `act.sub == ORCHESTRATOR_AGENT_ID` in the access token returned. Document whether `actor_token` header was required (if yes, Hop 1 implementation MUST do BFF code exchange via orchestrator backend, not via SPA). |
...
| **P12a** *(was P12, split)* | RFC 8693 chain: take a token whose `act.sub` is set, use it as `subject_token` in another token-exchange — does Asgardeo preserve nesting (`act.act.sub`) or flatten? | Either nested (good — depth-2 demo possible) or documented flatten (drop the stretch HR→IT chain). |
| **P12b** *(new)* | Pattern C composition: take a `requested_actor`-derived user token (Hop 1 output, has `act.sub=orchestrator-agent`), use it as `subject_token` in RFC 8693 exchange — what does the resulting token's `act` look like? | Same outcome categories as P12a; informs whether the Hop 1 → Hop 3 boundary preserves chain. |
| **P13** | Does `hr_server` accept tokens whose `aud` is the canonical hr_agent A2A URI? Or does hr_server's existing JWT validation reject them? | If reject (expected), hr_agent must re-mint via RFC 8693 for `hr_server`'s configured audience (Hop 4 in §3.3). Document hr_server's actual `EXPECTED_AUD`. |
| **P14** | What is `it_server`'s expected `aud` (defined at registration; e.g., `mcp://it_server.local`)? Does the chosen Asgardeo MCP Server registration for it_server reflect `aud` exactly? | Document `EXPECTED_AUD` for it_server; confirm RFC 8693 token-exchange targeting that resource yields the right `aud`. Mirrors P13 for the IT path. |
| **P15** *(new)* | Asgardeo's actor-token shape and lifetime: is it a JWT, what's its TTL, what's its `aud`/`scope`? What happens if it expires mid-flow at `/token` Pattern C step? | JWT shape documented; TTL recorded; refresh strategy chosen (re-run /authn 3-step on near-expiry). |
| **P16** *(new)* | Does Asgardeo apply rate-limiting / brute-force protection on `/oauth2/authn` for Agent identities? Are Agent Secrets accepted at the regular user-login authn path or restricted? | Sensible rate limit observed; Agent Secrets cannot be used as user credentials. |
| **P17** *(new)* | Agent identity revocation vs auto-created OAuth Application: are these separate operations? When you "Block Agent" or "Delete Agent" in Console, what happens to the OAuth App's tokens? | Document both surfaces. Sprint 2 revocation tests must hit the right one. |
| **P18** *(new)* | When a user's session is revoked (logout / admin terminate), do tokens whose `act.sub=<some agent>` (Pattern C / RFC 8693 derived) get invalidated, or do they survive until natural exp? | Document cascade behavior. Drives whether Sprint 2's Layer A (introspection) is the safety net or BCL is. |
| **P19** *(new)* | When the orchestrator-agent's actor_token is revoked, do tokens minted using it as `actor_token` (in Hops 3a/3b) get invalidated? | Documents whether revocation propagates through the token-exchange chain. |
...
- `hr_agent/` — new specialist that wraps `hr_server` via MCP. Owns A2A endpoint + agent card.
- `it_agent/` — new specialist that wraps `it_server` via MCP. Owns A2A endpoint + agent card.
- `it_server/` — **new backend** mirroring `hr_server/` shape (FastAPI + FastMCP). In-memory asset fixture (5–10 sample rows). Validates `aud=<it_server canonical URI>`, scope `it_assets_read_mcp`, nested `act` allowlist (Sprint 1 allowlists `it_agent` AND `orchestrator-agent` so the chain user→orchestrator→it_agent→it_server validates).
...
`docker-compose.yml` updated with the new services. Ports: orchestrator 8080, hr_agent 8001, it_agent 8002, hr_server 8003, **it_server 8004**.
...
2. `docs/spikes/asgardeo-capability-memo.md` is committed with all probes recorded (P0, P1–P5, P7, P8, P10.A/B, P11, P12a, P12b, P13–P19; P6, P9 deferred) — signed off by lead + security-engineer.
3. `common/auth/` and `common/a2a/` packages are committed and consumed by `hr_server/` (introspection feature-flagged, default OFF; agent-card unused yet but importable).
...
6. Empty service stubs (`orchestrator/`, `hr_agent/`, `it_agent/`) exist and `docker compose up` builds (won't yet do anything useful).
...
3. **Agent identity registration**
    - `orchestrator-agent`, `hr_agent`, `it_agent` — three Agent identities. Capture client_id / client_secret for each.
    - Each agent must be **assigned to roles** (gotcha #D) so it can mint tokens with appropriate scopes.
4. **API Resources + MCP Servers** — Asgardeo distinguishes the two registration types under the API Resources section. Use API Resource for agent-tier; **MCP Server** for backend-tier (the architecturally honest choice).
    - `HR Agent API` (regular API Resource; audience = `https://hr.smart-employee.local/a2a`), scopes `hr_basic_a2a`, `hr_self_a2a`, `hr_read_a2a`, `hr_approve_a2a`.
    - `IT Asset Agent API` (regular API Resource; audience = `https://it.smart-employee.local/a2a`), scope `it_assets_read_a2a`.
    - `HR Server` (MCP Server; audience = `mcp://hr_server.local`), scopes `hr_basic_mcp`, `hr_self_mcp`, `hr_read_mcp`, `hr_approve_mcp` *(matches existing hr_server code)*.
    - `IT Server` (MCP Server; audience = `mcp://it_server.local`), scope `it_assets_read_mcp`.
...
| `hr_agent/` | Specialist. Owns `/a2a/message:send`, `/.well-known/agent-card.json`. Wraps `hr_server` MCP for tool dispatch. | New **Agent identity**: `hr_agent` + new **API Resource** `HR Agent API` with `hr_*_a2a` scopes. |
| `it_agent/` | Specialist. Same pattern. Wraps `it_server/` via MCP for tool dispatch (parallel to HR Agent → hr_server). | New **Agent identity**: `it_agent` + new **API Resource** `IT Asset Agent API` with `it_assets_read_a2a` scope. |
| `it_server/` | New backend. FastAPI + FastMCP, in-memory asset store (5–10 sample rows). Mirrors `hr_server/` shape. | New **MCP Server** `IT Server` with `it_assets_read_mcp` scope. |

### 3.3 Token flow

```
─── Hop 1: User → SPA → Orchestrator (Pattern C delegated-agent OBO) ──
  Per Asgardeo agentic-AI docs (Pattern C):
  Browser ── PKCE → Asgardeo /authorize?
              client_id=<orchestrator-app SPA client_id>&
              requested_actor=<ORCHESTRATOR_AGENT_ID>&
              scope=openid+agent_access+hr_basic_a2a+hr_self_a2a+...+it_assets_read_a2a&
              code_challenge=...&redirect_uri=http://localhost:3001/callback
            (consent screen rendered by Asgardeo: "Delegate to Orchestrator Agent? [Approve][Deny]")
  Browser → SPA (client/, port 3001) /callback?code=...
  SPA → Orchestrator backend (port 8090) POST /api/auth/exchange  body={code, code_verifier}
            (the BFF pattern — orchestrator backend, NOT the SPA, performs code exchange,
             so the orchestrator-agent's actor_token can be attached in Authorization header
             per Pattern C "agent attaches its actor_token in Authorization header")
  Orchestrator backend → Asgardeo /token
                          headers: Authorization: Bearer <orchestrator_actor_token from Hop 2>
                          body: grant_type=authorization_code, client_id=<SPA>, code,
                                code_verifier, redirect_uri
            ⇒ user_delegated_token: {sub: user, act: {sub: ORCHESTRATOR_AGENT_ID},
                                      aud: <orchestrator-app aud>, scope: <requested>}
  Orchestrator stores user_delegated_token in its own session (cookie to SPA).

─── Hop 2: Orchestrator → Asgardeo (3-step App-Native Auth — mint actor token; cached) ──
  Per Asgardeo agent-authentication docs: agents auth via /oauth2/authn, NOT client_credentials.
  Step 2.1: POST /oauth2/authorize  (response_mode=direct,
                                      client_id=<orchestrator-agent OAuth App client_id>,
                                      response_type=code, redirect_uri=...,
                                      scope=openid+internal_login,
                                      resource=<self> or omitted,
                                      code_challenge=...)   → flowId + nextStep[BasicAuth]
  Step 2.2: POST /oauth2/authn      (flowId, selectedAuthenticator:
                                      {authenticatorId: <BasicAuthenticator base64 ID>,
                                       params: {username: <ORCHESTRATOR_AGENT_ID>,
                                                password: <ORCHESTRATOR_AGENT_SECRET>}})
                                                          → code (+ state, session_state)
  Step 2.3: POST /oauth2/token      (grant_type=authorization_code, client_id=<OAuth App>,
                                      code, code_verifier, redirect_uri)
                                                          → orchestrator_actor_token
  ⇒ orchestrator_actor_token: {sub: orchestrator-agent (the Agent ID),
                                 aud: per resource= or app default, iss: ddademo issuer}

─── Hop 3a: Orchestrator → HR Agent (per-call exchange; agent-tier scopes) ──
  Orchestrator → Asgardeo /token  (RFC 8693 token-exchange,
              ...
              scope=hr_read_a2a+hr_approve_a2a)   ← _a2a tier (validated by hr_agent)
            ⇒ hr_call_token: {sub: user, act.sub: orchestrator-agent,
                              aud: https://hr.smart-employee.local/a2a,
                              scope: hr_read_a2a hr_approve_a2a}
  Orchestrator → HR Agent /a2a/message:send
              Authorization: Bearer <hr_call_token>
              Body: {"jsonrpc":"2.0","method":"message/send", ...}
  HR Agent: validate sig+iss+aud(exact)+exp+scope(_a2a tier)+act.sub(allowlist)
            → re-mint to call hr_server (Hop 4)

─── Hop 3b: Orchestrator → IT Agent (per-call exchange; agent-tier scopes) ──
  Symmetric: aud=https://it.smart-employee.local/a2a, scope=it_assets_read_a2a

─── Hop 4: HR Agent → hr_server (backend re-mint; A2A→MCP scope transform) ──
  HR Agent → Asgardeo /token  (RFC 8693 token-exchange,
              subject_token=hr_call_token (carries hr_*_a2a scopes),
              actor_token=hr_agent's actor token (from 3-step App-Native Auth flow),
              resource=<hr_server EXPECTED_AUD verified by P13>,
              scope=hr_read_mcp+hr_approve_mcp)   ← _a2a (agent-tier) tokens
                                                    are exchanged for _mcp
                                                    (backend-tier) scopes here.
                                                    The substitution IS the
                                                    security boundary.
            ⇒ hr_server_token: {sub: user, act.sub: hr_agent,
                                  act.act.sub: orchestrator-agent,
                                  aud: <hr_server canonical URI>}
  HR Agent → hr_server MCP  Authorization: Bearer <hr_server_token>
  hr_server: validate sig+iss+aud(exact, =EXPECTED_AUD)+exp+scope+nested act
            → existing tool dispatch

─── Hop 5: IT Agent → it_server (parallel to Hop 4) ─────────────────────
  IT Agent → Asgardeo /token  (RFC 8693 token-exchange,
              subject_token=it_call_token (carries it_assets_read_a2a),
              actor_token=it_agent's actor token (from 3-step App-Native Auth flow),
              resource=mcp://it_server.local  (verified by P14),
              scope=it_assets_read_mcp)   ← _a2a → _mcp transform (see scope-policy.md §1)
            ⇒ it_server_token: {sub: user, act.sub: it_agent,
                                  act.act.sub: orchestrator-agent,
                                  aud: mcp://it_server.local}
  IT Agent → it_server MCP  Authorization: Bearer <it_server_token>
  it_server: validate sig+iss+aud(exact)+exp+scope+nested act
            → in-memory asset store dispatch
```

**Hop 4 and Hop 5 demonstrate the same security property on two paths:** the agent → backend re-mint pattern with full 3-name `act` chain (`user`/`orchestrator`/`specialist-agent`). v2 had hr_agent forwarding the incoming token to hr_server, which would either fail (token's `aud` doesn't match hr_server's `EXPECTED_AUD`) or pass permissively (worse). v3 has each specialist re-mint per RFC 8693 for its backend's exact resource URI. Spikes P13 (hr_server) and P14 (it_server) verify each backend's `EXPECTED_AUD` so the resource URI in the exchange call is correct.
...
2. Register `orchestrator-agent`, `hr_agent`, `it_agent` as Agent identities. Issue client credentials.
3. Register two API Resources (`HR Agent API`, `IT Asset Agent API` — agent-tier) and two MCP Servers (`HR Server`, `IT Server` — backend-tier) with their respective scopes.
4. Configure `requested_actor` permitted-actors policy: `orchestrator-app` may use `requested_actor=orchestrator-agent`.
5. Token-exchange happens via `orchestrator-agent` (Agent identity), NOT `orchestrator-app` (SPA). Subscribe `orchestrator-app` and the three Agent identities to the four resources (2 API Resources + 2 MCP Servers) so all the right scope sets and resource targets are reachable for the relevant hops.

**C — Orchestrator extraction (`orchestrator/`)**
6. Move chat endpoint, session store, LangChain integration, Gemini wiring from `agent/` to `orchestrator/`. **Drop direct `hr_server` MCP wiring** — orchestrator no longer talks to `hr_server`; it talks to `hr_agent` via A2A.
7. Update `client/app.js`: login URL builder appends `requested_actor=<ORCHESTRATOR_AGENT_ID>`. Add a small UI hint on the login button: "(delegating chat to Orchestrator Agent)". **Stop calling `/oauth2/token` directly from the SPA** if Pattern C P10.B applies — instead, after receiving the auth code at `/callback`, POST it to the orchestrator backend at `/api/auth/exchange` (next task) and let the backend complete the code exchange.

7a. **NEW: Orchestrator BFF code-exchange endpoint** (`POST /api/auth/exchange` on `orchestrator/main.py`). Required only if P10.B applies (Pattern C requires `actor_token` attached at /token). Contract:
    - Request: `Content-Type: application/json`, body `{"code": str, "code_verifier": str, "redirect_uri": str}`
    - Server-side action: POST `<ASGARDEO_BASE>/oauth2/token` with `grant_type=authorization_code`, `client_id=<ORCHESTRATOR_APP_CLIENT_ID>`, `code`, `code_verifier`, `redirect_uri`, AND `Authorization: Bearer <orchestrator_actor_token>` header (the actor token from task 8). Asgardeo returns the user-delegated token.
    - Response: HTTP 200 with `Set-Cookie: orchestrator_session=<sid>; HttpOnly; Secure; SameSite=Lax`. Body: `{"user": {"sub": str, "name": str}, "expires_in": int}`. The user-delegated token is stored server-side, never returned to the SPA.
    - Errors: 400 (invalid code/verifier), 502 (Asgardeo unreachable), 401 (Asgardeo rejected the exchange — usually means `requested_actor` policy denial or actor_token expired).
    - If P10.A applies (no actor_token needed), this endpoint still exists but skips the Authorization header — keeps client SPA code uniform regardless of probe outcome.
8. Implement **3-step App-Native Auth client** (`orchestrator/actor_token.py`) — REPLACES the `client_credentials` design. Per Asgardeo's documented agent authentication, the orchestrator-agent's actor token is minted via:
   - POST `/oauth2/authorize` with `response_mode=direct`, `client_id=<ORCHESTRATOR_AGENT_OAUTH_CLIENT_ID>`, PKCE → `flowId`.
   - POST `/oauth2/authn` with `flowId` + `selectedAuthenticator: {authenticatorId, params: {username: <ORCHESTRATOR_AGENT_ID>, password: <ORCHESTRATOR_AGENT_SECRET>}}` → `code`.
   - POST `/oauth2/token` with `grant_type=authorization_code`, `code`, `code_verifier` → access_token.
   - Cache the resulting token (used as `actor_token` in Hops 1, 3a, 3b). Refresh at 80% of TTL with jitter. Negative cache 30 s on errors.
   - Implementation: a `FlowBasedActorTokenClient` class wrapping the 3 sequential httpx POSTs. Verified by spike P0.
...
**D — HR Agent specialist (`hr_agent/`)**
...
16. **Re-mints token via RFC 8693 to call hr_server (Hop 4 in §3.3)** — does NOT forward the incoming token. `subject_token` = incoming `hr_call_token`. **`actor_token` = hr_agent's actor token, minted via the same 3-step App-Native Auth flow described in task 8** but using `<HR_AGENT_OAUTH_CLIENT_ID>` + `<HR_AGENT_ID>` + `<HR_AGENT_SECRET>` (cached + refreshed). `resource` = hr_server's verified `EXPECTED_AUD`, `scope` = subset needed for the skill (e.g., `hr_read_mcp` for read tools).
...
19. **Application JSON-RPC error code catalog** (consistent across hr_agent and it_agent):
...
19a. Inbound requests carry an `X-Correlation-Id` header (UUIDv4). hr_agent forwards it into the downstream MCP call as metadata; logs it; echoes it back in JSON-RPC `data.correlation_id`. New histogram `a2a_to_mcp_dispatch_latency_seconds`.

**E — IT Asset Agent specialist (`it_agent/`)**
20. Scaffold mirroring `hr_agent/`. **Wraps `it_server/` via MCP (`langchain-mcp-adapters`)** for tool dispatch — does NOT hold asset data itself. Architecture is symmetric with HR Agent → hr_server.
...
22. **Hop 5 re-mint** before calling it_server: RFC 8693 token-exchange with `subject_token`=incoming `it_call_token`, **`actor_token`=it_agent's actor token (minted via the 3-step App-Native Auth flow with `<IT_AGENT_OAUTH_CLIENT_ID>` + `<IT_AGENT_ID>` + `<IT_AGENT_SECRET>`)**, `resource`=it_server `EXPECTED_AUD` (verified by P14), `scope=it_assets_read_mcp`. Same caching + singleflight semantics as Hop 4.
...
**E2 — IT backend (`it_server/`)** — new in v3
23a. **Scaffold IDENTICAL to `hr_server/`** in shape and conventions — directory layout, FastAPI + FastMCP wiring, in-memory store pattern (mirror `hr_server/service/store.py`), config loading, error envelopes. The point of `it_server/` is symmetry with `hr_server/`; deviations would muddle the demo. In-memory data: 5–10 sample rows `{asset_id, employee_id, type, model, status}`. MCP tools: `get_employee_assets(employee_id, asset_category=None, limit=50, cursor=None)` returning `{"assets": [...], "total": int, "next_cursor": str|null}`; `get_asset_by_id(asset_id)`.
23b. JWT validation per `common/auth.jwt_validator`:
    - Required `aud`: exact `mcp://it_server.local` (or whatever the configured `EXPECTED_AUD` resolves to from P14).
    - Required scope: `it_assets_read_mcp`.
    - **Nested `act` chain check**: walk every level; allowlist for it_server is `IT_SERVER_TRUSTED_PEER_AGENTS=it_agent,orchestrator-agent` (chain depth 2: it_agent on top, orchestrator below).
...
24. **Surgical** LangSmith redaction (not the nuclear `LANGSMITH_HIDE_INPUTS=true` switch): a `hide_inputs`/`hide_outputs` callable masks:
    - **Bearers**: `Authorization` header value, `actor_token`, `subject_token`, `client_assertion`, anything matching `eyJ[A-Za-z0-9_-]{10,}\.`, any field whose key contains `token`.
    - **3-step App-Native Auth flow-state secrets** (added per security review of the new mint mechanism): `flowId`, `code`, `code_verifier`, `selectedAuthenticator` (since `params.password` is the Agent Secret), and any field whose key matches `password|secret`.
    - Tool args, messages, and outputs remain visible.
...
29. **N1 — Bearer-forward rejected.** Orchestrator forwards the user delegated token directly (no exchange) to HR. Expected: 401, `error: invalid_audience` (delegated token has `aud=orchestrator-app`, not `hr_agent-api`).

29a. **N1b — DROPPED** (was: `act` stripped post-Pattern-C). Per security review: tampering an `act` claim breaks the JWT signature, so signature-failure preempts the missing-`act` rule under test. The intended scenario — a *signature-valid* token with `act` absent — is already covered by **N6** (which sources such a token from a misconfigured fallback path). N1b is therefore a duplicate; folded into N6.
...
34. **N6 — Act-removal / chain-loss enforcement.** Test that the validator's primary defense is the **missing-`act` rule**, not signature failure. Mint a signature-valid token (e.g., via a misconfigured fallback path) whose `act` claim is absent. Expected: 401, validator rejects with `error: invalid_token` citing missing actor chain — proving the validator enforces the chain even when the signature passes.

34a. **N6b — Agent-only token (no user delegation) rejected.** Use the orchestrator-agent's actor token directly (the output of P0/Hop 2, which has `sub=orchestrator-agent`, no user, no `act`). Present to a specialist. Expected: 401 — specialist requires `sub=user` AND non-empty `act`. Proves agent-only credentials cannot bypass user delegation; user consent is mandatory.
...
36. **N8 — Spoofed agent card.** Stand up a malicious server claiming to be `it_agent` (different URL). Orchestrator's discovery is locked to a configured allowlist of agent-card URLs (env var `ORCHESTRATOR_AGENT_CARD_URLS`); cards from other origins are ignored before body parsing. Expected: malicious card not loaded.
...
45a. **N16 — Specialist forwards instead of re-mints.** HR Agent forwards the incoming `hr_call_token` (aud=hr_agent-api) directly to hr_server's MCP without re-minting. Expected: hr_server rejects with `invalid_audience` (-32001). Repeat the test for IT Agent → it_server path: same rejection at it_server. Both paths must fail; this is what proves the agent→backend re-mint is enforced, not optional.
45b. **N17 — Wrong backend audience.** Take a token correctly minted for hr_server (aud=`mcp://hr_server.local`) and replay it against it_server. Expected: it_server rejects with `invalid_audience`. Cross-backend replay must fail just as cross-specialist replay does (N7).

Identity-propagation:

46. **N13 — Token + flow-state leakage check.** Inspect the full LangSmith run tree (root + all child runs, all messages, all error fields, agent-card content) for:
    - Any string matching `Bearer\s` or `eyJ[A-Za-z0-9_-]{10,}\.` (bearers + JWT bodies)
    - Any field with key matching `flowId|code_verifier|password|secret` exposed in inputs/outputs (3-step App-Native Auth flow-state)
    - Agent-card `url` and `auth` blocks (must be stripped by surgical redactor)
    Expected: zero matches across all three categories.
...
- Sequence diagram checked in to `docs/diagrams/orchestrator-a2a-flow.md` (mermaid) — must show **all five hops** including hr_agent → hr_server (Hop 4) and it_agent → it_server (Hop 5) re-mints.
...
| **P12 fail** (no chained-actor preservation across exchanges) | Skip the stretch HR→IT specialist-to-specialist demo. Hop 4 (hr_agent → hr_server) still works because chain depth there is 2 — verify in P12 specifically that depth-2 is supported even if depth-3 isn't. |
| **P13 fail** (hr_server rejects the hr_agent-aud token AND hr_server cannot be reconfigured) | Reconfigure hr_server's `EXPECTED_AUD` to accept the hr_agent-aud (still permissive only to the exact value), OR add a thin shim to hr_server that re-introspects each token. Re-mint (Hop 4) is the preferred path; fallbacks documented in spike memo. |
...
**Layer A — Per-request introspection with BCL-driven cache-bust** (specialists: hr_agent, it_agent).
...
- On valid logout token: `sessions.terminate_by_sid(sid)` → cache-bust dispatcher fires to hr_agent + it_agent → best-effort `/oauth2/revoke` for the user's OBO refresh token.
...
| `hr_agent/auth/`, `it_agent/auth/` | Switch validator to JWKS-then-introspect chain. Add `POST /internal/auth/cache-bust` endpoint (HMAC-protected). HR feature-flagged `HR_INTROSPECT_ENABLED=true` in this sprint; IT was already on. |
...
- `docker-compose.yml` declares `deploy.replicas: 1` for orchestrator (and for now, hr_agent / it_agent). CI assertion verifies the constraint so R7 and BCL tests are deterministic.
...
|---|---|---|
| **M0 — Sprint 0 done** | Spike memo (**P0, P1, P2, P3, P4, P5, P7, P8, P10.A/P10.B, P11, P12a, P12b, P13, P14, P15, P16, P17, P18, P19** — 18+ probes; P6 and P9 deferred per v3 POC review) committed; `common/auth/` + `common/a2a/` lands; scope policy + agent-card schema + UX validation spec + Asgardeo setup guide committed; library pins verified; service stubs build (incl. it_server); `agent/` archived to `_archive/agent.before-v3/` and tagged `pre-v3-orchestrator`. | Lead engineer + security-engineer sign-off. |
